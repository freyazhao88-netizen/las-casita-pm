...
-- Labor subcontracts: a lump-sum paid for a scope of work (e.g. foundation), not by
-- the day — counts toward a project's labor cost but is never tied to an employee
-- record or the day-based attendance/payroll system.
create table if not exists labor_subcontracts (
  id bigint generated by default as identity primary key,
  project_id bigint references projects(id) on delete cascade,
  adhoc_project_name text default '',
...
-- Small Jobs (one-off jobs with no real Project record) are purely a derived rollup
-- of payments/materials/expenses/attendance sharing the same typed name — this table
-- is the one place to attach extra info (contact, phone, address, notes) to such a
-- job, keyed by a case/whitespace-normalized name so it matches regardless of how
-- the name was capitalized on any given entry.
